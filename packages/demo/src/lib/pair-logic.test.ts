...
describe("setPair", () => {
  it("no-ops when a === b (no self sub-grid)", () => {
    ...
    setPair(pair, { a: 1, i: 0, b: 1, j: 2 }, "confirmed", "user");
  ...
  it("mirrors writes so pair[a][i][b][j] === pair[b][j][a][i]", () => {
    const pair = emptyPair([4, 4]);
    setPair(pair, { a: 0, i: 1, b: 1, j: 2 }, "confirmed", "auto");
...
describe("subgridUniquenessRule", () => {
  it("returns no writes when the changed cell is not confirmed", () => {
    const pair = emptyPair([4, 4]);
    set(pair, { a: 0, i: 0, b: 1, j: 0 }, "eliminated");
  ...
  it("skips already-non-empty cells", () => {
    const pair = emptyPair([4, 4]);
    set(pair, { a: 0, i: 0, b: 1, j: 0 }, "confirmed");
    set(pair, { a: 0, i: 0, b: 1, j: 1 }, "eliminated"); // pre-existing — should not be re-emitted
    const writes = subgridUniquenessRule.derive(pair, {
      a: 0,
      i: 0,
      b: 1,
...
describe("deriveCrossSubgrids", () => {
  ...
  it("returns no writes when pinned-axis knowledge is empty", () => {
    ...
    expect(deriveCrossSubgrids(pair, 0)).toHaveLength(0);
  });
  ...
  it("confirms a cross pair when both values share a confirmed pinned position", () => {
    ...
    set(pair, { a: 1, i: 0, b: 0, j: 1 }, "confirmed");
    set(pair, { a: 2, i: 2, b: 0, j: 1 }, "confirmed");
    const writes = deriveCrossSubgrids(pair, 0);
  ...
  it("eliminates a cross pair when every pinned position rules one out", () => {
    ...
    set(pair, { a: 1, i: 0, b: 0, j: 0 }, "confirmed");
    set(pair, { a: 2, i: 0, b: 0, j: 1 }, "confirmed");
    // Sub-grid uniqueness would normally fill the rest; simulate by eliminating the other positions
    for (let p = 1; p < 4; p++)
      set(pair, { a: 1, i: 0, b: 0, j: p }, "eliminated");
    for (let p = 0; p < 4; p++)
      if (p !== 1) set(pair, { a: 2, i: 0, b: 0, j: p }, "eliminated");
    const writes = deriveCrossSubgrids(pair, 0);
  ...
  it("leaves a cross pair alone when pinned-axis knowledge is ambiguous", () => {
    ...
    set(pair, { a: 1, i: 0, b: 0, j: 2 }, "eliminated");
    set(pair, { a: 1, i: 0, b: 0, j: 3 }, "eliminated");
    set(pair, { a: 2, i: 0, b: 0, j: 1 }, "eliminated");
    set(pair, { a: 2, i: 0, b: 0, j: 3 }, "eliminated");
    const writes = deriveCrossSubgrids(pair, 0);
  ...
  it("retracts a cross-sub-grid derivation when the supporting pinned fact is removed", () => {
    ...
    function derive(pair: PairState): DerivedWrite[] {
      return deriveCrossSubgrids(pair, 0);
    }

    const pair = emptyPair(sizes);
    set(pair, { a: 1, i: 0, b: 0, j: 1 }, "confirmed"); // catA=0 at pin 1
    set(pair, { a: 2, i: 2, b: 0, j: 1 }, "confirmed"); // catB=2 at pin 1
    ...
    set(pair, { a: 1, i: 0, b: 0, j: 1 }, "empty");
...
describe("recomputeAuto", () => {
  ...
  it("never clobbers a user-set eliminate even if a rule would derive the same cell", () => {
    const pair = emptyPair(sizes);
    set(pair, { a: 1, i: 0, b: 0, j: 1 }, "confirmed"); // user confirm
    set(pair, { a: 1, i: 0, b: 0, j: 2 }, "eliminated"); // user elim (sub-grid uniqueness would derive this)
    recomputeAuto(pair, 0);
  ...
  it("never clobbers a user-set eliminate when a rule would derive CONFIRMED on the same cell", () => {
    ...
    set(pair, { a: 1, i: 0, b: 0, j: 0 }, "confirmed"); // catA=0 at pin 0
    set(pair, { a: 2, i: 0, b: 0, j: 0 }, "confirmed"); // catB=0 at pin 0
    // But the user asserts the opposite cross pair.
    set(pair, { a: 1, i: 0, b: 2, j: 0 }, "eliminated");
    recomputeAuto(pair, 0);
  ...
  it("cross-only user confirms stay local — no derivation without pinned-axis support", () => {
    ...
    set(pair, { a: 1, i: 0, b: 2, j: 0 }, "confirmed");
    recomputeAuto(pair, 0);
...
describe("replaceConfirm", () => {
  ...
  it("wipes user confirms in the same sub-row before asserting the new one", () => {
    const pair = emptyPair(sizes);
    set(pair, { a: 1, i: 0, b: 0, j: 0 }, "confirmed"); // user confirm: catA=0 at pin 0
    replaceConfirm(pair, { a: 1, i: 0, b: 0, j: 1 }); // user now asserts catA=0 at pin 1
  ...
  it("wipes user confirms in the same sub-col before asserting the new one", () => {
    const pair = emptyPair(sizes);
    set(pair, { a: 1, i: 0, b: 0, j: 1 }, "confirmed"); // user confirm: catA=0 at pin 1
    replaceConfirm(pair, { a: 1, i: 2, b: 0, j: 1 }); // user now asserts catA=2 at pin 1
